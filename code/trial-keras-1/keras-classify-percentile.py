...
def nn_model():
    model = Sequential()
    model.add(Dense(512, input_dim = xtrain.shape[1], init = 'he_normal'))
...
callbacks = [
    ...
    # ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=3, verbose=1, mode='auto', epsilon=0.0001, cooldown=0, min_lr=0)
    ]

## compute percentiles
perc_step_size = 7
perc_values = np.percentile(y, range(perc_step_size, 101, perc_step_size))
perc_groups = len(perc_values)
y_perc = np.zeros_like(y)
for v in perc_values[:-1]:
    y_perc += (y > v)
median_each_perc = np.zeros((perc_groups,))
for i in range(perc_groups):
    median_each_perc[i] = np.median(y[y_perc == i])
...
folds = StratifiedKFold(y_perc, n_folds = nfolds, shuffle = False, random_state = None)
for j in range(nbags):
    for i, (inTr, inTe) in enumerate(folds):
        ...
        p = model.predict_generator(generator = batch_generatorp(xte, 800, False), val_samples = xte.shape[0])[:,0]
        p = median_each_perc[p.round().clip(0,perc_groups-1).astype(int)]
        print('Bag ', j, ' Fold ', i, ' - MAE:', mean_absolute_error(y[inTe], p))
        pred_oob[inTe] += p
        p = model.predict_generator(generator = batch_generatorp(xtest, 800, False), val_samples = xtest.shape[0])[:,0]
        p = median_each_perc[p.round().clip(0,perc_groups-1).astype(int)]
        pred_test += p 
